...
<flux:with-tooltip :$tooltip :$attributes>
    <flux:button-or-link :attributes="$attributes->class($classes)" :$as :$href data-flux-avatar data-slot="avatar" data-size="{{ $size }}">
        <?php if ($src): ?>
            <img src="{{ $src }}" alt="{{ $alt ?? $name }}" class="rounded-full size-full object-cover">
        <?php elseif ($icon): ?>
